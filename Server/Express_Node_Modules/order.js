...
//router.post('/addorder/:p_id/:user_id',upload.single('thumbnail'), (request, response) => {
    ...
    const {p_id,user_id,total_amount} = request.body
    //const {order_date,deliverd_date} = request.body
    const connection = db.connect()
    const statement = `insert into order_details(p_id,user_id,order_date,deliverd_date,total_amount) values('${p_id}','${user_id}','${order_date}','${deliverd_date}',${total_amount})`
...
router.post('/placeorder',upload.single('thumbnail'), (request, response) => {
    const {user_id,total_amount} = request.body
    //const {order_date,deliverd_date} = request.body
    const connection = db.connect()
    const statement = `insert into order_details(user_id,order_date,deliverd_date,total_amount) values('${user_id}','${order_date}','${deliverd_date}',${total_amount})`
    // connection.query(statement, (error, data) => {
    //     connection.end()
    //     response.send(utils.createResult(error, data))
    // })
...
router.get('/:user_id', (request, response) => {
    ...
    connection.query(statement, (error, data) => {
        ...
        router.get('/orderdetails/:user_id', (request, response) => {
            const {user_id} = request.params
            const connection = db.connect()
            //const statement=`select * from order_details where user_id=${user_id}`
            //const statement = `select a.*,p.image,l.lc_product_name,(p.weight_per_garm*(g.daily_rate_per_gram+l.lc_charge))*3 as Total from add_to_cart a inner join product p on a.p_id=p.p_id inner join labour_chart l on p.lc_product_code=l.lc_product_code inner join gold_rate g on l.purity=g.purity where a.email_id='${email_id}'`
            const statement = `select p.image,l.lc_product_name,o.order_code,o.order_date,o.deliverd_date,(p.weight_per_garm*(g.daily_rate_per_gram+l.lc_charge))*3 as Total from order_details o inner join user_info u on o.user_id=u.user_id inner join product p on o.p_id=p.p_id inner join labour_chart l on p.lc_product_code=l.lc_product_code inner join gold_rate g on l.purity=g.purity where o.user_id='${user_id}'`
            //const statement = `select p.image,o.order_code,o.order_date,o.deliverd_date from order_details o inner join user_info u on o.user_id=u.user_id where o.user_id=${user_id}`;
            connection.query(statement, (error, data) => {
