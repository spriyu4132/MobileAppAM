...
//showing the product to android as well as angular. 
...
//showing only eraring product
router.get('/earring', (request, response) => {
    const connection = db.connect()
    //const statement =`select * from product`
    //const statement = `select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code`
    const statement=`select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity,(p.weight_per_garm*(g.daily_rate_per_gram+l.lc_charge))*3 as Total from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code inner join gold_rate g on g.purity=l.purity where category_id='er01' or category_id='er02'`
    connection.query(statement, (error, data) => {
        connection.end()
        response.send(utils.createResult(error, data))
    }) 
...
//showing only ring product
router.get('/ring', (request, response) => {
    const connection = db.connect()
    //const statement =`select * from product`
    //const statement = `select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code`
    const statement=`select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity,(p.weight_per_garm*(g.daily_rate_per_gram+l.lc_charge))*3 as Total from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code inner join gold_rate g on g.purity=l.purity where category_id='ring01'`
    connection.query(statement, (error, data) => {
        connection.end()
        response.send(utils.createResult(error, data))
    }) 
...
//showing only bangles product
router.get('/bracelet', (request, response) => {
    const connection = db.connect()
    //const statement =`select * from product`
    //const statement = `select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code`
    const statement=`select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity,(p.weight_per_garm*(g.daily_rate_per_gram+l.lc_charge))*3 as Total from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code inner join gold_rate g on g.purity=l.purity where category_id='bang01'`
    connection.query(statement, (error, data) => {
        connection.end()
        response.send(utils.createResult(error, data))
    }) 
...
//showing only mangalsutra/necklace product
router.get('/necklace', (request, response) => {
    const connection = db.connect()
    //const statement =`select * from product`
    //const statement = `select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code`
    const statement=`select p.*,l.lc_product_name,l.lc_charge,l.gst,l.type_of_making,l.purity,(p.weight_per_garm*(g.daily_rate_per_gram+l.lc_charge))*3 as Total from product p inner join labour_chart l on p.lc_product_code=l.lc_product_code inner join gold_rate g on g.purity=l.purity where category_id='mangal01'`
    connection.query(statement, (error, data) => {
        connection.end()
        response.send(utils.createResult(error, data))
    }) 
...
//showing the product to angular (user). 
...
//showing the product details of specific product to angular(user). 
...
//adding the product into database from admin side
...
//updating the specific product by admin
...
//deleting the specific product by admin
